...
namespace Eaton_DG_PCC.Model
{
    public class SQLHelper
    {
        ...
        //连接字符串
        static string strConn1 = ConfigurationManager.ConnectionStrings["CnnhoRechargePlatformConnectionString"].ToString();
        static string strConn = @"Data Source=test-PC\AndyLi;Initial Catalog=PCTron;User ID=sa;Password=ok;";
        //static string strConn = @"Data Source=.;Initial Catalog=PCTron;User ID=sa;Password=ok;";
        ...
        public static int ExecuteSqlTran(Hashtable SQLStringList)
        {
            ...
            {
                conn.Open();
                using (SqlTransaction trans = conn.BeginTransaction()) //启动事务
                {
